...
// Helper functions to generate components based on item type
...
func generateWardrobeComponents(length, width uint) []model.Component {
	// Perform calculations once and reuse results
	lengthDiv2 := length / 2
	widthDiv2 := width / 2
	lengthDiv3 := length / 3
	...
	return []model.Component{
		{
			...
			BackSidePanel: fmt.Sprintf("Wardrobe Back Panel (Length: %d, Width: %d)", length, widthDiv2),
			SidePanel:     fmt.Sprintf("Wardrobe Side Panel (Length: %d, Width: %d)", lengthDiv2, width),
			TopPanel:      fmt.Sprintf("Wardrobe Top Panel (Length: %d, Width: %d)", lengthDiv3, widthDiv3),
			BottomPanel:   fmt.Sprintf("Wardrobe Bottom Panel (Length: %d, Width: %d)", lengthDiv4, widthDiv4),
